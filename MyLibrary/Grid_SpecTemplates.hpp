/*
 * Grid_SpecTemplates.hpp
 *
 *  Created on: Jun 22, 2011
 *      Author: marchi
 */

#ifndef GRID_SPECTEMPLATES_HPP_
#define GRID_SPECTEMPLATES_HPP_
...
template<>
vector<double> Grid<1>::Rdf(FILE * fp,const double x[DIM], const string comm, const double & cut, const double & dx){
	...
	for(int i=0;i<nx0;i++){
		...
		for(int j=0;j<ny0;j++){
			...
			for(int k=0;k<nz0;k++){
				...
				if(dist < cut){
					int h=(int) (dist/dx);
					rdf[h]+=(dist<4.0)?DV*(*this)[0][i][j][k]:0.0;
					irdf[h]++;
				}
	...
	oss<<"@  s"<<Grid<DIM>::SetNo << " legend " << "\"" << comm << "\"" << std::endl;
...
template<>
void Grid<1>::RdfK(FILE * fp, const string comm, const double & cut, const double & dx){
	printf("dx = %f \n",dx);
	int nrdf=(int) (cut/dx)+1;
	double * rdf=new double [nrdf];
	int * irdf=new int [nrdf];
...
template<>
vector<double> Grid<DIM>::Rdf(FILE * fp,const double x[DIM], const string comm, const double & cut, const double & dx){
	double xa[DIM];
	int nrdf=(int) (cut/dx)+1;
		...
		for(int j=0;j<ny0;j++){
			...
			for(int k=0;k<nz0;k++){
				...
				if(dist != 0.0) {
					...
					if(dist < cut  ){
						int h=(int) (dist/dx);
						op=(*this)[XX][i][j][k]*vr[XX]+(*this)[YY][i][j][k]*vr[YY]+(*this)[ZZ][i][j][k]*vr[ZZ];
						rdf[h]+=op;
						irdf[h]++;
					}
				}
...
template <>
void Grid<1>::Xplor(FILE * fp, double convf){
	int nx_s,ny_s,nz_s;
	int nx_e,ny_e,nz_e;
	...
	for(int i=0; i< nx0; i++){
		for(int j=0; j< ny0; j++){
			for(int k=0; k< nz0; k++){
				...
				avg+=MyGrid[0][i][j][k]*convf;
				avg2+=MyGrid[0][i][j][k]*MyGrid[0][i][j][k]*convf2;
			}
	...
	for(int k=0;k<nz0;k++){
		...
		for(int j=0;j<ny0;j++){
			for(int i=0;i<nx0;i++){
				ja++;
				fprintf(fp,"%12.5E",MyGrid[0][i][j][k]*convf);
				if(!(ja%6)) fprintf(fp,"\n");
			}
		}
		fprintf(fp,"\n");
...
Grid<DIM>  Grid<1>::Der1(){
	...
	array4<double> dfi(DIM,nnx,nny,nnz);
	...
	Backward3.fftNormalized(dgi[XX],dfi[XX]);
	Backward3.fftNormalized(dgi[YY],dfi[YY]);
	Backward3.fftNormalized(dgi[ZZ],dfi[ZZ]);
 	return dfi;
}
template<>
Grid<DIM>  Grid<1>::Der2(){
...
Grid<1>  Grid<DIM>::Div(){
	...
	for(unsigned i=0;i<nnx;i++)
		for(unsigned j=0;j<nny;j++)
			for(unsigned k=0;k<nnz;k++)
				temp[0][i][j][k]=rhor[i][j][k];
 	return temp;
}
template<>
	Grid<1> Grid<1>::Derivative(unsigned int Dir){
	Grid<1> temp;
	return temp;
}


#endif /* GRID_SPECTEMPLATES_HPP_ */
